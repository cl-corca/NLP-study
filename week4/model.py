...
class RecursiveDeviceModule(nn.Module):
    """Module to recursively move all sub-modules to a specified device."""
...
class MultiHeadAttention(RecursiveDeviceModule):
    ...
    def forward(self, x: Tensor, mask: Tensor, encoder_output: Tensor | None = None):
        attention_outputs = [attention(x, x, x, mask) for attention in self.attentions]
...
class GPT(RecursiveDeviceModule):
    ...
    def get_optimizer(self):
        decay_set, no_decay_set = set(), set()
    ...
    def forward(self, x: Tensor):
        mask = self.look_ahead_mask(self.padding_mask(x))
        ...
        return self.fc_head(self.layer_norm(x))
    ...
    def generate(
        ...
        for _ in range(max_iter):
            x = x if x.size(1) <= self.block_size else x[:, -self.block_size :]
